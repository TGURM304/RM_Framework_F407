...
void MotorController::update(double target) {
    ...
    if(error_code & APP_MOTOR_ERROR_TIMEOUT) activate(false), error_code ^= APP_MOTOR_ERROR_TIMEOUT;
    ...
    if(use_extend_angle) {
        if(use_degree_angle) angle -= calc_delta(360, cur_angle_, lst_angle_);
        else angle -= calc_delta(8192, cur_angle_, lst_angle_);
    ...
    for(auto &[fn, controller] : pipeline_) {
        if(fn == nullptr) result = controller->update(this, result);
        else result = controller->update(fn(this), result);
    }
...
void MotorController::add_controller(const std::function<float(const MotorController *)> &fn,
                                     std::unique_ptr<Controller::Base> controller) {
