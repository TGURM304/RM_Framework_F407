...
DMMotor::DMMotor(const char *name, const Model &model, const Param &param)
: model_(model)
, param_(param) {
...
void DMMotor::control(float position, float speed, float Kp, float Kd, float torque) const {
    ...
    uint16_t P_des = float_to_uint(position, -param_.p_max, param_.p_max, 16),
             V_des = float_to_uint(speed, -param_.v_max, param_.v_max, 12), Kp_ = float_to_uint(Kp, 0, 500, 12),
             Kd_ = float_to_uint(Kd, 0, 5, 12), T_ff = float_to_uint(torque, -param_.t_max, param_.t_max, 12);
